...
public class AgentManagerController extends BaseController {
	...
	public String getAll(final User user, @RequestParam(value = "region", required = false) final String region, ModelMap model) {
		...
		Collection<AgentInfo> filterAgentByCluster = Collections2.filter(agents, new Predicate<AgentInfo>() {
			@Override
		...
		Collection<AgentInfo> filterAgentByUserAuthorityAndId  = Collections2.filter(filterAgentByCluster, new Predicate<AgentInfo>() {
			@Override
			public boolean apply(AgentInfo agentInfo) {
		...
		model.addAttribute("agents", filterAgentByUserAuthorityAndId);
	...
	private boolean filterAgentByUserAuthorityAndId(Collection<? extends GrantedAuthority> authorities, String userId, String region, String agentRegion) {
		if (adminOrSuperUser(authorities)) {
			return true;
		}
		if (containsAuthority(authorities, "U")) {
			if (region == null) {
	...
	/**
	 * Check current user is admin or superuser
	 */
	private boolean adminOrSuperUser(Collection<? extends GrantedAuthority> authorities) {
